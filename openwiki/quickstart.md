...
Cargo workspace (`rust-version = 1.82`, edition 2021) with four crates:
...
Requirements: Windows 10/11, Rust 1.82+, Visual Studio Build Tools (C++ linker) if linking fails.
...
- **Release packaging renames** the admin release binary to `poltergeist-admin.exe` in the zip; Cargo still emits `poltergeist.exe`.
